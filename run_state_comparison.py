...
def run_state_analysis(state_name, load, solar_base, wind_base):
    ...
    # Apply state-specific generation mix
    ...
    savings_pct = ((cost_no_bess - res['total_cost']) / cost_no_bess) * 100
    peak_reduction_pct = ((max_demand_no_bess - res['peak_demand']) / max_demand_no_bess) * 100
    
    return {
        "State": state_name,
        "Cost": res['total_cost'],
        "Savings%": savings_pct,
        "PeakRed%": peak_reduction_pct,
...
def main():
    print("--- STATE-AWARE BUSINESS ANALYSIS ---")
    ...
    print(f"Reason: {'Net Metering Enabled' if best['NetMetering'] else 'Aggressive Peak Shaving'} and favorable tariffs.")
