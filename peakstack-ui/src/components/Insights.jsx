...
export default function Insights({ 
  ...
  const safeRiskFactors = realism.risk_factors || [];
          ...
          <div className="chart-header">
           ...
           <div className="realism-content" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <p style={{ fontSize: '14px', color: 'var(--text-muted)', lineHeight: '1.5' }}>
               {realism.confidence_reason ?? 'Analysis based on historical patterns and current tariff structures.'}
              </p>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px', background: 'var(--bg-main)', borderRadius: '12px', border: '1px solid var(--border)' }}>
                 <div>
                   ...
                   <div style={{ fontSize: '20px', fontWeight: '800' }}>{formatCurrency(realism.theoretical_savings_inr ?? 0)}</div>
                 </div>
                 ...
                 <div>
                   ...
                   <div style={{ fontSize: '20px', fontWeight: '800', color: 'var(--accent)' }}>{formatCurrency(realism.realistic_savings_inr ?? 0)}</div>
                 </div>
              </div>
 
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                 {safeRiskFactors.map(risk => (
          ...
          <div className="insights-list" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              return (
                <div key={i} style={{ display: 'flex', gap: '16px', alignItems: 'flex-start', borderBottom: '1px solid var(--border)', paddingBottom: '12px' }}>
                  <div style={{ 
                    ...
                    <span>{(insight.time || 'N/A').split(' ')[0] || 'N/A'}</span>
                    <span>{(insight.time || '').split(' ')[1] || ''}</span>
                  </div>
                  
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '14px', lineHeight: '1.4', marginBottom: '4px' }}>
                      {insight.explanation ?? 'No insight details available.'}
                    </div>
                    {insight.impact_percent > 0 && (
                      <div style={{ fontSize: '12px', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '6px' }}>
                        ...
                        Primary driver impact: <span style={{ fontWeight: '600' }}>{(insight.impact_percent ?? 0).toFixed(0)}%</span>
                      </div>
                    )}
                  </div>
                </div>
            ...
            {safeInsights.length === 0 && (
                <div style={{ fontSize: 13, color: 'var(--text-muted)', padding: 20, textAlign: 'center' }}>
                    No critical events detected in the analysis period.
                </div>
            )}
          </div>
        </div>
